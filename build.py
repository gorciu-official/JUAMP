...
if '--debug' in args or '-d' in args:
    ...
    CC.append('-Wshadow')
    CC.append('-Wformat')
    CC.append('-Wformat-security')
    CC.append('-Wpointer-arith')
    CC.append('-Wcast-qual')
    CC.append('-Wwrite-strings')
    CC.append('-Wundef')
for arg in args:
...
try:
    ...
    def link(objFiles: list[Path]):
        ...
        def compilationEffects(objs, stop_signal, completed_effects):
            for i, obj in enumerate(objs):
                if stop_signal.is_set():
                    completed_effects.extend(objs[i:])
                    break

                file = obj.name

                padding = (maxlen - len(file)) + 3
                arrow = "\033[1m║\033[0m" if i < len(objs) - 1 else "🮦"
